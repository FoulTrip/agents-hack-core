    ...
    try:
        ...
        if e.status == 404:
            ...
            if user_name == user.login:
                logger.info(f"Repo {repo_full_name} not found, creating it...")
                try:
                    ...
                    logger.error(f"Failed to create repo {repo_full_name}: {create_e}")
                    raise e
            else:
                logger.error(f"Repo {repo_full_name} not found and cannot create under different user")
                raise e
        else:
            raise
